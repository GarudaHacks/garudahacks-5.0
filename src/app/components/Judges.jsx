    ...
    const cardData = [
      {
        ...
        bio: "Matt Sokoloff is an AI Engineer at Google with a passion for using technology to solve real-world problems. While pursuing a finance degree, an internship in Silicon Valley sparked a fascination with programming, leading him to build his own deep learning computer. This fueled his expertise, ultimately inspiring him to earn a graduate degree in computer science. As an early employee at Arturo, an insurtech company spun out of American Family Insurance, he had the opportunity to develop models for analyzing property data from satellite and aerial imagery. Later, at Labelbox, he tackled challenges in large-scale data processing and analysis, playing a key role in shaping the platform's technical architecture.", 
      },
      {
        id: 2,
      ...
      {
        id: 18,
        url: "/imgs/abstract/img19.jpeg",
        name: "Dzung",
        description: "Software Engineer, AV Simulation at Cruise",
        bio: "Dzung currently works at Cruise, developing simulation tools for testing self-driving cars. She graduated from the University of Pennsylvania with a bachelor’s degree in Computer Graphics. Dzung was born in Vietnam, spent 7 years in Missouri before moving to Pennsylvania for college. She loves coffee, food, particularly sweets, and likes to cook for her friends and family.", 
      }
    ];

    setCards(cardData);

    const cardUrls = cardData.map(card => card.url);
    preloadImages(cardUrls);
  }, []);

  return (
    <section className="bg-black py-10 max-w-7xl w-full mx-auto display:flex">
      <h2 className="text-white text-center text-3xl font-bold pb-10">Judges</h2>
      <div className="grid grid-cols-min-content md:grid-cols-3 lg:grid-cols-3 gap-20 px-4">
        {cards.map((card) => (
